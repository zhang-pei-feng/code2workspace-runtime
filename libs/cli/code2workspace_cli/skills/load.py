...
code2workspace.middleware.skills and adapts it for direct filesystem access
needed by CLI commands.

For middleware usage within agents, use
code2workspace.middleware.skills.SkillsMiddleware directly.
"""
...
def list_skills(
    *,
    ...
    1. `user_skills_dir` (`~/.code2workspace/{agent}/skills/`)
    2. `user_agent_skills_dir` (`~/.agents/skills/`)
    3. `project_skills_dir` (`.code2workspace/skills/`)
    ...
    Args:
        ...
        user_skills_dir: Path to `~/.code2workspace/{agent}/skills/`.
        project_skills_dir: Path to `.code2workspace/skills/`.
...
def load_skill_content(
    ...
    *,
    ...
    if allowed_roots and not any(path.is_relative_to(root) for root in allowed_roots):
        ...
        from code2workspace_cli._env_vars import EXTRA_SKILLS_DIRS
