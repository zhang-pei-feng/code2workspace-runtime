...
class ProjectContext:
    ...
    def project_skills_dir(self) -> Path | None:
        """Return the project `.code2workspace/skills` directory, if any."""
        if self.project_root is None:
            return None
        return self.project_root / ".code2workspace" / "skills"

    def project_agents_dir(self) -> Path | None:
        """Return the project `.code2workspace/agents` directory, if any."""
        if self.project_root is None:
            return None
...
def find_project_agent_md(project_root: Path) -> list[Path]:
    ...
    1. project_root/.code2workspace/AGENTS.md
    ...
    """
    candidates = [
        project_root / ".code2workspace" / "AGENTS.md",
