...
_REMOTE_TOOL_LOAD_TIMEOUT_SECONDS = 15.0
"""Timeout for loading tools from a remote MCP server during server bootstrap."""
...
def discover_mcp_configs(
    *, project_context: ProjectContext | None = None
    ...
    1. `~/.code2workspace/.mcp.json` (user-level global)
    2. `<project-root>/.code2workspace/.mcp.json` (project subdir)
    ...
    """
    user_dir = Path.home() / ".code2workspace"
    project_root = _resolve_project_config_base(project_context)

    candidates = [
        user_dir / ".mcp.json",
        project_root / ".code2workspace" / ".mcp.json",
...
def classify_discovered_configs(
    ...
    User-level configs live under `~/.code2workspace/`. Everything else is
...
async def _load_tools_from_config(
    ...
    *,
    ...
    tool_load_timeout: float | None = None,
    ...
    Args:
        ...
        persistent_sessions: Whether to keep MCP sessions open across tool calls.
        best_effort: Whether to skip individual servers that fail tool discovery.
        tool_load_timeout: Optional timeout in seconds for per-server tool loading.
    ...
    try:
        ...
        for server_name, server_config in config["mcpServers"].items():
            try:
                ...
                timeout = None
                if (
                    tool_load_timeout is not None
                    and _resolve_server_type(server_config) in _SUPPORTED_REMOTE_TYPES
                ):
                    timeout = tool_load_timeout

                tools = (
                    await asyncio.wait_for(load_coro, timeout=timeout)
                    if timeout is not None
                    else await load_coro
                )
            except Exception as e:
                if best_effort:
                    logger.warning(
                        "Skipping MCP server '%s' after tool load failure: %s",
                        server_name,
                        e,
                        exc_info=True,
                    )
...
async def get_mcp_tools(
    ...
    if manager is None:
        msg = "Persistent MCP session manager was not created"
        raise RuntimeError(msg)
...
async def resolve_and_load_mcp_tools(
    *,
    ...
    Args:
        ...
        persistent_sessions: When `True`, keep MCP sessions open across tool
            calls. When `False`, discover tool schemas with a temporary session
            and reopen a fresh session for each MCP tool invocation. This is
            safer for server bootstraps that cross event loop boundaries.
        best_effort: Whether to skip individual servers that fail tool discovery.
        tool_load_timeout: Optional timeout in seconds for per-server tool loading.
    ...
    return await _load_tools_from_config(
        ...
        tool_load_timeout=tool_load_timeout,
    )
