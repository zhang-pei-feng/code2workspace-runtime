...
def _resolve_editable_info() -> tuple[bool, str | None]:
    ...
    try:
        dist = distribution("code2workspace-cli")
...
class Settings:
    ...
    code2workspace_langchain_project: str | None
    """LangSmith project name for code2workspace agent tracing."""
    ...
    def from_environment(cls, *, start_path: Path | None = None) -> Settings:
        ...
        return cls(
            ...
            code2workspace_langchain_project=code2workspace_langchain_project,
    ...
    def reload_from_environment(self, *, start_path: Path | None = None) -> list[str]:
        ...
        reloadable_fields = (
            ...
            "code2workspace_langchain_project",
            "project_root",
            "shell_allow_list",
            "extra_skills_dirs",
        )
        ...
        # the change
        new_project = refreshed["code2workspace_langchain_project"]
        if new_project:
            os.environ["LANGSMITH_PROJECT"] = new_project
        elif previous["code2workspace_langchain_project"]:
    ...
    def get_project_agent_md_path(self) -> list[Path]:
        ...
        Checks both `{project_root}/.code2workspace/AGENTS.md` and
        ...
        `.code2workspace/AGENTS.md` first.
        ...
        if not self.project_root:
            return []
        from code2workspace_cli.project_utils import find_project_agent_md
    ...
    def get_project_skills_dir(self) -> Path | None:
        ...
        Returns:
            Path to {project_root}/.code2workspace/skills/, or None if not in a project
        """
        if not self.project_root:
            return None
        return self.project_root / ".code2workspace" / "skills"
    ...
    def get_project_agents_dir(self) -> Path | None:
        ...
        Returns:
            Path to {project_root}/.code2workspace/agents/, or None if not in a project
        """
        if not self.project_root:
            return None
        return self.project_root / ".code2workspace" / "agents"
...
_OPENROUTER_APP_URL = "https://pypi.org/project/code2workspace-cli/"
...
def _get_provider_kwargs(
    ...
    """
    from code2workspace_cli.model_config import ModelConfig
...
def validate_model_capabilities(model: BaseChatModel, model_name: str) -> None:
    """Validate that the model has required capabilities for `code2workspace`.
