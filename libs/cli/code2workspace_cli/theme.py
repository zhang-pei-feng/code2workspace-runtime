...
`register_theme()` in `Code2WorkspaceApp.__init__`, while the few app-specific
...
Users can define custom themes in `~/.code2workspace/config.toml` under
...
def reload_registry() -> MappingProxyType[str, ThemeEntry]:
    ...
    Re-reads `~/.code2workspace/config.toml` for user-defined themes so that
