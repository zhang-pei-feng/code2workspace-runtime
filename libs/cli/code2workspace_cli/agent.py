...
if TYPE_CHECKING:
    ...
    from code2workspace.backends.sandbox import SandboxBackendProtocol
    from code2workspace.middleware.async_subagents import AsyncSubAgent
    from code2workspace.middleware.subagents import CompiledSubAgent, SubAgent
...
from code2workspace_cli.integrations.sandbox_factory import get_default_working_dir
...
class ShellAllowListMiddleware(AgentMiddleware):
    ...
    **before execution**. Rejected commands are returned as error `ToolMessage`
    ...
    def _validate_tool_call(self, request: ToolCallRequest) -> ToolMessage | None:
        ...
        from code2workspace_cli.config import SHELL_TOOL_NAMES, is_shell_command_allowed
...
def load_async_subagents(config_path: Path | None = None) -> list[AsyncSubAgent]:
    ...
    if config_path is None:
        config_path = Path.home() / ".code2workspace" / "config.toml"
...
def list_agents(*, output_format: OutputFormat = "text") -> None:
    ...
    """
    agents_dir = settings.user_code2workspace_dir
    ...
    if not agents_dir.exists() or not any(agents_dir.iterdir()):
        ...
        console.print(
            "[dim]Agents will be created in ~/.code2workspace/ "
...
def get_system_prompt(
    ...
    *,
    ...
    skills_path = f"~/.code2workspace/{assistant_id}/skills"
...
def create_cli_agent(
    ...
    *,
    ...
    # read the channel.
    from code2workspace_cli.token_state import TokenStateMiddleware
