...
class TestResolveAndLoadMcpTools:
    ...
    async def test_explicit_path_merges_with_discovery(
        ...
        assert mock_load.call_args.kwargs["persistent_sessions"] is True
    ...
    async def test_persistent_sessions_flag_is_forwarded(
        ...
        """Callers can opt out of persistent sessions for server bootstraps."""
        c1 = tmp_path / "project.json"
        c1.write_text(
            json.dumps({"mcpServers": {"docs": {"type": "http", "url": "http://ok"}}})
        )
        mock_discover.return_value = [c1]
        mock_load.return_value = ([], None, [])

        await resolve_and_load_mcp_tools(persistent_sessions=False)

        mock_load.assert_awaited_once()
        assert mock_load.call_args.kwargs["persistent_sessions"] is False
        assert mock_load.call_args.kwargs["best_effort"] is False
        assert mock_load.call_args.kwargs["tool_load_timeout"] is None
