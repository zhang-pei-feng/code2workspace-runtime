"""Wall-time benchmarks for `create_workspace_agent` graph construction.
...
`create_workspace_agent` under various configurations. They do NOT invoke the
...
if TYPE_CHECKING:
    from code2workspace.middleware.subagents import SubAgent
...
def _build_kwargs(**overrides: Any) -> dict[str, Any]:
    """Build default kwargs for `create_workspace_agent`, merged with overrides."""
...
class TestCreateWorkspaceAgentBenchmark:
    """Wall-time benchmarks for `create_workspace_agent` graph construction."""
    ...
    def test_create_workspace_agent_minimal(self, benchmark: BenchmarkFixture) -> None:
        """Baseline: no user-supplied tools, subagents, or middleware."""
        kwargs = _build_kwargs()

        @benchmark  # type: ignore[misc]
        def _() -> None:
            create_workspace_agent(**kwargs)
    ...
    def test_with_string_model_resolution(self, benchmark: BenchmarkFixture) -> None:
        ...
        fake = _fake_model()
        with patch("code2workspace.graph.resolve_model", return_value=fake):
...
@pytest.mark.benchmark
class TestCreateWorkspaceAgentScaling:
