...
from code2workspace.middleware.skills import (
...
def test_create_workspace_agent_with_skills_and_filesystem_backend(tmp_path: Path) -> None:
    """Test end-to-end: create_workspace_agent with skills parameter and FilesystemBackend."""
    # Create skill on filesystem
    backend = FilesystemBackend(root_dir=str(tmp_path), virtual_mode=False)
    skills_dir = tmp_path / "skills" / "user"
    skill_path = str(skills_dir / "test-skill" / "SKILL.md")
    skill_content = make_skill_content("test-skill", "A test skill for workspace agents")
...
def test_create_workspace_agent_with_skills_empty_directory(tmp_path: Path) -> None:
...
def test_create_workspace_agent_with_skills_default_backend() -> None:
    """Test create_workspace_agent with skills parameter using default backend (no backend specified).
