...
from code2workspace.backends.protocol import EditResult, ReadResult, WriteResult
from code2workspace.backends.store import BackendContext, StoreBackend, _NamespaceRuntimeCompat, _validate_namespace
...
def test_store_backend_legacy_path_rejects_malicious_assistant_id() -> None:
    ...
    for bad_id in malicious_ids:
        ...
        with patch("code2workspace.backends.store.get_config", return_value=fake_cfg), warnings.catch_warnings(record=True):
            warnings.simplefilter("always")
            with pytest.raises(ValueError, match="disallowed characters"):
                be.write("/test.txt", "content")
