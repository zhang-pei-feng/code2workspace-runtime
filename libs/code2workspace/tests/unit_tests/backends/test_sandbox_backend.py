...
from code2workspace.backends.protocol import (
    ExecuteResponse,
    FileDownloadResponse,
    FileUploadResponse,
)
from code2workspace.backends.sandbox import (
...
class MockSandbox(BaseSandbox):
    ...
    def execute(self, command: str, *, timeout: int | None = None) -> ExecuteResponse:
        ...
        # Detect temp-file upload path: upload_files() stores .code2workspace_edit_*
        # keys in _file_store before execute() is called.
        has_tmp = any(".code2workspace_edit_" in k for k in self._file_store)
...
def test_sandbox_edit_upload_cleans_up_temp_files() -> None:
    ...
    assert result.error is None
    assert not any(k.startswith("/tmp/.code2workspace_edit_") for k in sandbox._file_store)  # noqa: S108
...
def test_sandbox_edit_upload_malformed_output_cleans_up() -> None:
    ...
    assert ".code2workspace_edit_" in cleanup_commands[0]
