"""End-to-end unit tests for code2workspace with fake LLM models."""
...
from code2workspace.backends import CompositeBackend, FilesystemBackend
from code2workspace.backends.protocol import BackendProtocol, ExecuteResponse, SandboxBackendProtocol
from code2workspace.backends.state import StateBackend
from code2workspace.backends.store import StoreBackend
from code2workspace.backends.utils import TOOL_RESULT_TOKEN_LIMIT, create_file_data
from code2workspace.graph import create_workspace_agent
from code2workspace.middleware.filesystem import NUM_CHARS_PER_TOKEN
from code2workspace.middleware.permissions import FilesystemPermission
from code2workspace.middleware.subagents import SubAgent  # noqa: TC001
...
from tests.utils import SampleMiddlewareWithTools, SampleMiddlewareWithToolsAndState, assert_all_workspace_agent_qualities
...
class TestWorkspaceAgentEndToEnd:
    """Test suite for end-to-end workspace-agent functionality with fake LLM."""

    def test_workspace_agent_with_fake_llm_basic(self) -> None:
        """Test basic workspace-agent functionality with a fake LLM model.

        This test verifies that a workspace agent can be created and invoked with
        ...
        # Create a workspace agent with the fake model
    ...
    def test_tool_runtime_config_includes_default_graph_metadata(self) -> None:
        """Test tool runtime config includes the defaults from `create_workspace_agent`."""
        ...
        assert captured_config["metadata"]["ls_integration"] == "code2workspace"
        assert captured_config["metadata"]["lc_agent_name"] == "supervisor"
        assert "code2workspace" in captured_config["metadata"]["versions"]

    def test_workspace_agent_with_fake_llm_with_tools(self) -> None:
        """Test workspace_agent with tools using a fake LLM model.

        This test verifies that a workspace_agent can handle tool calls correctly
        ...
        # Create a workspace agent with the fake model and sample_tool
        agent = create_workspace_agent(model=model, tools=[sample_tool])
    ...
    def test_workspace_agent_with_fake_llm_filesystem_tool(self) -> None:
        """Test workspace_agent with filesystem tools using a fake LLM model.

        This test verifies that a workspace_agent can use the built-in filesystem
    ...
    def test_workspace_agent_with_fake_llm_multiple_tool_calls(self) -> None:
        """Test workspace_agent with multiple tool calls using a fake LLM model.

        This test verifies that a workspace_agent can handle multiple sequential
    ...
    def test_workspace_agent_with_string_model_name(self) -> None:
        """Test that create_workspace_agent resolves string model names correctly.
        ...
        with patch("code2workspace.graph.resolve_model", return_value=fake_model):
            agent = create_workspace_agent(model="claude-sonnet-4-6", tools=[sample_tool])
    ...
    def test_workspace_agent_truncate_lines(self, tmp_path: Path, backend: BackendProtocol) -> None:
    ...
    def test_workspace_agent_read_empty_file(self, tmp_path: Path, backend: BackendProtocol) -> None:
    ...
    def test_workspace_agent_with_system_message(self) -> None:
        """Test that create_workspace_agent accepts a SystemMessage for system_prompt."""
        model = FixedGenericFakeChatModel(
            messages=iter(
                [
        ...
        agent = create_workspace_agent(model=model, system_prompt=system_msg, middleware=[capturing_middleware])
        assert_all_workspace_agent_qualities(agent)
        ...
        assert "You are a Workspace Agent" in content

    def test_workspace_agent_with_system_message_string_content(self) -> None:
        """Test that create_workspace_agent accepts a SystemMessage with string content."""
        model = FixedGenericFakeChatModel(
            messages=iter(
                [
    ...
    def test_workspace_agent_two_turns_no_initial_files(self) -> None:
        """Test workspace_agent with two conversation turns without specifying files on invoke.
    ...
    def test_workspace_agent_two_turns_state_backend_edge_case(self) -> None:
    ...
    async def test_workspace_agent_two_turns_no_initial_files_async(self) -> None:
        """Async version: Test workspace_agent with two conversation turns without specifying files.
    ...
    async def test_workspace_agent_two_turns_state_backend_edge_case_async(self) -> None:
    ...
    def test_workspace_agent_read_file_truncation(self, tmp_path: Path, backend: BackendProtocol) -> None:
    ...
    def test_workspace_agent_read_file_no_truncation_small_file(self, tmp_path: Path, backend: BackendProtocol) -> None:
    ...
    def test_workspace_agent_read_file_truncation_with_offset(self, tmp_path: Path, backend: BackendProtocol) -> None:
    ...
    async def test_workspace_agent_read_file_truncation_async(self, tmp_path: Path, backend: BackendProtocol) -> None:
    ...
    def test_workspace_agent_read_file_single_long_line_behavior(self, tmp_path: Path, backend: BackendProtocol) -> None:
    ...
    def test_read_large_single_line_file_returns_reasonable_size(self) -> None:
        ...
        agent = create_workspace_agent(model=fake_model)
    ...
    def test_workspace_agent_read_file_invalid_args_returns_tool_message(self, tmp_path: Path, backend: BackendProtocol) -> None:
        ...
        agent = create_workspace_agent(model=fake_model, backend=backend)
    ...
    def test_workspace_agent_read_image_file(self, tmp_path: Path, backend: BackendProtocol) -> None:
...
class TestWorkspaceAgentPermissionsEndToEnd:
    """End-to-end tests for create_workspace_agent with FilesystemPermission."""
...
class TestWorkspaceAgentStructure:
    """Test basic workspace agent structure without making network calls."""

    def test_base_workspace_agent(self) -> None:
        """Verifies that a basic workspace agent can be created with default settings."""
        agent = create_workspace_agent()
        assert_all_workspace_agent_qualities(agent)

    def test_workspace_agent_with_tool(self) -> None:
        """Verifies that a workspace agent can be created with tools and the tools are properly bound."""
        agent = create_workspace_agent(tools=[sample_tool])
        assert_all_workspace_agent_qualities(agent)
        assert "sample_tool" in agent.nodes["tools"].bound._tools_by_name

    def test_workspace_agent_with_middleware_with_tool(self) -> None:
        """Verifies that middleware can inject tools into a workspace agent."""
        agent = create_workspace_agent(middleware=[SampleMiddlewareWithTools()])
        assert_all_workspace_agent_qualities(agent)
        assert "sample_tool" in agent.nodes["tools"].bound._tools_by_name

    def test_workspace_agent_with_middleware_with_tool_and_state(self) -> None:
        """Verifies that middleware can inject both tools and extended state channels."""
        agent = create_workspace_agent(middleware=[SampleMiddlewareWithToolsAndState()])
        assert_all_workspace_agent_qualities(agent)
        assert "sample_tool" in agent.nodes["tools"].bound._tools_by_name
...
class TestLargeHumanMessageEviction:
    ...
    def test_multi_turn_eviction(self) -> None:
        ...
        agent = create_workspace_agent(model=fake_model, checkpointer=InMemorySaver())
...
class TestSummarizationOffloadToState:
    ...
    def test_offloaded_file_persisted_in_state(self) -> None:
        ...
        Uses ``create_workspace_agent`` with default ``StateBackend`` so that
...
class TestArtifactsRoot:
    ...
    def test_workspace_agent_artifacts_root_system_prompt_and_eviction(self) -> None:
    ...
    def test_workspace_agent_artifacts_root_eviction_then_read(self) -> None:
    ...
    def test_workspace_agent_artifacts_root_conversation_history_offload(self) -> None:
    ...
    def test_create_workspace_agent_no_composite_backend(self) -> None:
        """create_workspace_agent with a non-composite backend defaults artifacts_root to '/'."""
        backend = StateBackend()
        capturing_middleware = SystemMessageCapturingMiddleware()
        agent = create_workspace_agent(
    ...
    def test_create_workspace_agent_composite_backend_default_artifacts_root(self) -> None:
        """create_workspace_agent with CompositeBackend without artifacts_root defaults to '/'."""
    ...
    def test_human_message_eviction_uses_artifacts_root(self) -> None:
        ...
        agent = create_workspace_agent(model=model, backend=backend, checkpointer=InMemorySaver())
...
class TestAsyncSubagentEndToEnd:
    ...
    These tests wire up ``create_workspace_agent`` with ``AsyncSubAgent`` specs and
    ...
    @patch("code2workspace.middleware.async_subagents.get_sync_client")
