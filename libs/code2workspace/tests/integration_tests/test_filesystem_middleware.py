...
from code2workspace.backends import CompositeBackend, StateBackend, StoreBackend
...
from code2workspace.middleware.filesystem import (
    FileData,
...
class TestFilesystem:
    ...
    def test_longterm_memory_multiple_tools_workspace_agent(self):
        checkpointer = MemorySaver()
        store = InMemoryStore()
        ...
        agent = create_workspace_agent(backend=backend, checkpointer=checkpointer, store=store)
        assert_longterm_mem_tools(agent, store)

    def test_shortterm_memory_multiple_tools_workspace_agent(self):
        checkpointer = MemorySaver()
        store = InMemoryStore()
        agent = create_workspace_agent(backend=StateBackend(), checkpointer=checkpointer, store=store)
